...
$vote = $bdd->query('SELECT * FROM event_valide ORDER BY id_ev_v DESC ');
?>
<!DOCTYPE html>
<html lang="fr" >
<head>
...
<div class="container">
    ...
    <table class="table table-bordered" style="table-layout: fixed; word-wrap: break-word">
        ...
        <?php while($a = $vote->fetch()) { ?>
            <?php
            $number = $bdd->prepare('select COUNT(*) id_membre from nbr_inscription WHERE id_article=?');
            $number->execute(array($a['id_ev_v']));
            ...
            <tr>
                ...
                <td><?= $a['descr'] ?></td>
                <td><?= $t ?></td>
                <td><a style="color: #ef1a08;" href="delete_v_ev.php?id=<?= $a['id_ev_v'] ?>">Supprimer </a></td>
            </tr>
        <?php } ?>
        </tbody>
    </table>
